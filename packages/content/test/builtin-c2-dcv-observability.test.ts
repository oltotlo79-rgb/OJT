 ...
 * このテストは、内蔵C2課題のうち接点故障を持つ全題（c2-002・c2-003・c2-005・c2-007）について、
...
/** 接点の組（1〜4）の COM／NO 端子ID。 */
function contactTerminals(partId: string, elementIndex: number): { com: string; no: string } {
  // socketContactElementIndex(group, 'a') === group * 2（schema/faults.ts）。
  // 内蔵課題の接点故障は4件とも a接点（偶数の elementIndex）なので、ここでは a接点だけを扱う。
  expect(elementIndex % 2, `${partId} elementIndex`).toBe(0);
  const group = elementIndex / 2;
  const pins = SOCKET_CONTACT_PINS[group - 1];
  if (pins === undefined) throw new Error(`不正な接点の組: ${partId} elementIndex=${elementIndex}`);
  return { com: `${partId}.${String(pins.com)}`, no: `${partId}.${String(pins.no)}` };
}
