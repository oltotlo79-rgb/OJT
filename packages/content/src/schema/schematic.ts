...
/** 段の中の1要素。§11.1 */
export const SchematicCellSchema = z.strictObject({
  kind: CellKindSchema,
  id: z.string().min(1),
  ...
  presetMs: z
    ...
    .refine(hasExactTimerRange, {
      message:
        'タイマ設定値はレンジの刻みに載る値にします（0〜10秒は0.1秒刻み／0〜60秒は0.5秒刻み）',
    })
    .optional(),
});
...
export const RungEndSchema = z.union([
  z.strictObject({ bus: z.enum(['P', 'N']) }),
  z.strictObject({ rung: z.string().min(1), node: z.int().min(0) }),
]);
...
export function toZodPath(path: string): (string | number)[] {
  ...
  for (const token of path.split('.')) {
    ...
    /* c8 ignore next -- 正規表現が一致した時点で第1グループは必ず取れる（型のための既定値） */
    out.push(match[1] ?? token);
    /* c8 ignore next -- 第2グループは `*` なので一致時は空文字にはなっても undefined にならない */
