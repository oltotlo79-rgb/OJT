...
export interface PlcCoverMark {
  ...
  /**
   * 3Dでの見せ方（既定 `flat`）。決定表#16 の改訂（2026-09-26）
   * - `flat`: 蝶番の辺で {@link PLC_COVER_OPEN_DEG}（180°）まで開き、本体の外側に平らに寝かせる
   * - `removed`: 取り外した状態（描かない）。開くと表示灯の帯や隣のモジュールを塞ぐラック形のカバー
   *
   * どちらも机上の電線（z ≥ 9.6mm）の通り道に立たない。以前は100°で手前へ立てていたため、
   * 上から入る電線がカバーを突き抜けて見えた（2026-09-26 利用者報告「配線がシーケンサを貫通する」）。
   */
  open?: 'flat' | 'removed';
}
