...
/**
 * チャタリング判定の遷移回数しきい値。§5.3.2
 * しきい値は20で、1秒窓（`CHATTER_WINDOW_MS`）の中で同じ信号が20回以上反転したらチャタリングとみなす。
 * 正規のフリッカ回路のうち最速のもの（各タイマ休止100ms、周期200ms）でも反転は10回/秒にとどまり、
 * しきい値の半分なので誤検出しない。禁則回路（自己励振）は1tick（10ms）ごとに反転して100回/秒に達し、
 * しきい値を大きく超えるので必ず検出される。
 */
