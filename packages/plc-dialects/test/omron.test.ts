...
describe('OMRON CP1E のバリデータとスキン（§10.5 / §10.6）', () => {
  ...
  it('W446-E1-23 p161–162に沿って接点・描線・チェック・転送を割り当てる', () => {
    const keysOf = (action: string): string | undefined =>
      profile.shortcuts.find((row) => row.action === action)?.keys;
    expect(keysOf('contact-no')).toBe('C');
    expect(keysOf('contact-nc')).toBe('/');
    expect(keysOf('or-contact-no')).toBe('W');
    expect(keysOf('or-contact-nc')).toBe('X');
    expect(keysOf('coil')).toBe('O');
    expect(keysOf('instruction')).toBe('I');
    expect(keysOf('rule-line')).toBe('Ctrl+←↑↓→');
    expect(keysOf('convert')).toBe('F7,Ctrl+F7');
    expect(keysOf('monitor-toggle')).toBe('Ctrl+M');
    expect(keysOf('download')).toBe('Ctrl+T');
  });

  it('すべてのキーに確認した一次資料を対応させる', () => {
    for (const row of profile.shortcuts) {
      expect(row.confirmed, row.action).toBe(true);
      expect(row.source, row.action).toBe('S4');
    }
  });

  it('実機オンライン編集とシミュレータへの転送を混同しない', () => {
    const off = profile.shortcuts.filter((row) => row.enabled === false);
    expect(off.map((row) => row.action)).toEqual(['online-edit', 'online-send']);
    for (const row of off) expect(row.note).toContain('実機へのオンライン編集');
  });
