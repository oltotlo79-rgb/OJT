 ...
 * 確かめる。8本以外を1本でも足せば（あるいは `IPC_CHANNELS` にだけ足して登録を忘れれば）
 * ここで落ちる。preload が公開するのはこの表の8本だけなので、これが renderer から
...
describe('main が受け付ける IPC は §4.3 の8本だけ（DM-7）', () => {
  ...
  it('still counts eight channels (足すときは §4.3 と preload も直すこと)', () => {
    expect(Object.values(IPC_CHANNELS)).toHaveLength(8);
  });
});
