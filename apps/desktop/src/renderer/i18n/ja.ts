...
export const JA = {
  ...
  tour: {
    ...
    steps: {
      rotate: {
        title: '盤を回してみましょう',
        body: '左上のキューブをドラッグして回転します。盤面のドラッグは角度を保った移動、ホイールは拡大・縮小です。',
      },
      mount: {
        title: '部品をソケットに置きます',
      ...
      wire: {
        title: '端子どうしをつなぎます',
        body: '盤の端子を1つ押し、別の端子を押します。Altキーを押しながら端子間をドラッグしても配線できます。',
      },
      power: {
        title: '電気を流してみましょう',
  ...
  settings: {
    ...
    scaleNormal: '標準（100%）',
    scaleLarge: '大（115%）',
    scaleExtraLarge: '特大（130%）',
    contrast: '見やすさ',
    contrastNormal: '標準',
    contrastHigh: '高コントラスト',
  ...
  session: {
     ...
     */
    viewHint:
      '盤面を左／中／右ドラッグ＝平行移動　ホイール＝拡大・縮小　キューブをドラッグ＝回転　キューブの面・辺・角をクリック＝その視点へ　Alt＋端子ドラッグ＝配線　テンキー1/3/7＝正面/右/上（Ctrlで反対側）　Home＝全体',
    // --- view cube 2026-09-19 ---
  ...
  ladder: {
    ...
    editError: {
      rowLimit:
        'この回路ブロックの行数が上限（20行）に達しています。別の回路ブロックを追加してください。',
      lastRow: '最後の1行は削除できません。回路全体を消すには「ネットワーク削除」を使います。',
      lastRowLine: '最終行の下には罫線を引けません。先に行を追加してください。',
      duplicate: '回路ブロックを追加できませんでした。追加する位置を選び直してください。',
      missing: '選択していた回路ブロックがありません。編集する回路を選び直してください。',
      insertion: 'この位置には挿入できません。挿入する行を選び直してください。',
      selection: '選択していたマスがありません。編集するマスを選び直してください。',
    },
    // --- /Phase 7 Task 2 (LE-3) ---
  ...
  viewControls: {
    mouse: 'マウス',
    mouseHelp: '盤面を左・中・右ドラッグで移動。ホイールで拡大・縮小。Alt＋端子ドラッグで配線。',
    keyboard: 'キー',
  ...
  schematic: {
    ...
    branchCancel: '分岐をやめる',
    branchHint:
      'いま選んでいる段を、他の段の節点どうしをつなぐ「分岐」にします。自己保持回路に使います。',
    branchPickFrom:
      '分岐の始点をクリックしてください（他の段の、要素と要素のあいだを選びます）。Esc でやめられます。',
    branchPickTo:
      '分岐の終点をクリックしてください。始点から右側の節点を選ぶと、そのあいだの要素と並列になります。Esc でやめられます。',
    branchSelfRefused: '分岐の始点・終点には、他の段の節点を選んでください。',
  ...
  hoverHint: {
    ...
    idle: '盤面のドラッグで移動、キューブで回転、ホイールで拡大・縮小',
    ...
    /** 2本つながった端子を指している（3本目はつなげない。2026-09-26 利用者指示）。 */
    wireLimit: 'この端子にはすでに2本つながっています。3本目は接続できません',
  },
  /**
   * 3本目を配線しようとしたときの注意文（2026-09-26 利用者指示「3本目を配線しようとしたら
   * 注意文を出して」）。3Dの上に出し、閉じるか次の配線をするまで残す。
   */
  wireLimitNotice: {
    title: '注意：1つの端子に接続できる電線は2本までです',
    body: (label: string): string =>
      `${label} にはすでに2本つながっているため、3本目は接続できません。`,
    advice:
      '同じ電位へつなぐときは、この端子につながっている先の端子（渡り先）から配線してください。ねじ1か所に3本以上締めると、緩みや発熱の原因になります。',
    close: '閉じる',
  },
  // --- /Phase 7 Task 27 ---
  // --- Phase 7 Task 25 ---
 ...
 * 利用者要望 2026-09-20「リレーソケットの各番号はどこが何か分からないから分かるようにしてね」。
 *
