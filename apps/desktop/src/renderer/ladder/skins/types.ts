...
export interface SkinTheme {
  ...
  /** 公開資料でキーを確認できず、マウス入力を案内するメーカーか。 */
  keyMapAssumed?: boolean;
}

/** 4スキン共通の前提（「実物との対応」表の △ の理由）。 */
export const SKIN_ASSUMED: readonly string[] = [
  'メニュー順・入力手順・画面構成は公式資料で確認。OS・テーマ・倍率によって変わる色の数値やピクセル寸法は、読みやすさに合わせて調整している',
  '文字は12px以上、操作領域は32px以上。左ツリー・下部の出力欄は画面幅に合わせて配置する',
  '各社のロゴ・製品画像・マニュアルの図は同梱せず、記号を線画で描いている。表題の「風」は訓練用の再現画面を示す',
  '資料の版・ページと確認できていない項目は docs/reference/ladder-skin-sources.md に記録している',
  'PCwinの編集キーは未確認のため割り当てず、記号ボタンから入力する',
  '左の番号欄は回路ブロックの通し行数。本アプリは実機の命令ステップ数を保持しない',
];
