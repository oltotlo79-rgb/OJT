...
test('組立は配線ゼロから開始し、P端子から配線・測定・作業の切替ができる', async () => {
  ...
  try {
    ...
    await settledShot(app, page, 'v1.4.2-power-blank');
    ...
    await settledShot(app, page, 'v1.4.2-power-wired');
    ...
    await settledShot(app, page, 'v1.4.2-supply-detail');
    ...
    await expect(page.getByTestId('tester-readout')).toContainText('24');
    await settledShot(app, page, 'v1.4.2-direct-measurement');
    ...
    await page.waitForTimeout(350);
    await settledShot(app, page, 'v1.4.2-switch-detail-on');
    await page.getByTestId('power-switch').click();
    await page.getByTestId('power-breaker').click();
    await settledShot(app, page, 'v1.4.2-switch-detail-off');
    await page.keyboard.press('2');
    await page.waitForTimeout(400);
    await settledShot(app, page, 'v1.4.2-power-perspective');
  } finally {
    await app.close();
  }
});
