...
| §16 Phase 2 ③ | C2で故障2箇所を指摘し白線で修復すると合格 | 同上 | Task 18 / `test/sim-worker-repair-matrix.test.ts`・`test/inspect-repair-result-real-judge.test.tsx`（本物の `judgeInspectRepair()` を通す単体テスト）・`test/work-file-real-judge.test.ts`（保存→復元後も同じ判定になることを本物の判定で確認） |
...
| 2026-09-18 | Task 17 Step 5 実装時の再構成（プラン本文の案からの差分）: `toWorkFile()` は課題・盤・経過時間・危険操作数だけを引数に取り、`inspectFieldsFor()` が `useStore.getState()` を直接見て**自己充足的に**モード固有の項目（テスター・解答・指摘・故障・交換）を足す（プラン案は呼び出し側が個別に渡す形だった）。`restoreInspectState()` は課題を**受け取った状態に合わせて開き直す側**（`store.openProblem()` を自ら呼ぶ）で、呼び出し側は課題を先に開かない。`TesterState` は丸ごとではなく `SavedTester`（`kind`/`mode`/`voltRange`/`ohmRange`/`zeroAdjusted` の5項目、プローブ位置は除く）だけを持つ。プラン案にあった「`state.mode === undefined` をPhase 1ファイルとして常に許す」というC2の分岐は、`resolvedFaults` が無ければ判定できないため落とし、**C2は `mode` 欠落でも `resolvedFaults` が無ければ断る**形にした |
| 2026-09-18 | 最終レビュー（review-2b-final）の修正バッチ M3: **Blocking** B1（`replayTesterToWorker()` が `zero-adjust` を送り直しておらず、0Ω調整済みの復元後に Worker 側が未調整に戻っていた。末尾に `zero-adjust` を追加）／B2（`WorkerBridge.send()` は `start()` するまで no-op なので、クラッシュ復帰や課題切替での Worker 再起動時につまみの再送が捨てられていた。各画面の `load` 直後に `replayTesterToWorker()` を足した）。**Important** I1（`e2e/inspect.spec.ts` の固定 `waitForTimeout` を retrying assertion に置換）／I2（C2の不合格シナリオ―片方だけ指摘―を追加）。**Minor** M2（`toFaultSpecs()` が空配列を「欠落」と誤認していたのを修正。利用者課題の `faults: []` に対応）／M4（絞り込み結果0件とフォルダ空を別文言に）／M5（壊れたテスターの塊を静かに既定へ戻さずトーストで知らせる）／M6（main の `parseWorkFile()` が配列を `tester` として誤って通していたのを `!Array.isArray()` で修正）。テストはレビュアーの探り（`review-2b-final/*.probe.ts`）を `work-file-inspect.test.ts` / `work-file-restart-resend.test.tsx` / `work-file-real-judge.test.ts` へ lift。desktop 単体テストは 58 files/798 tests → 60 files/808 tests、E2E 17本（新規の不合格シナリオを含む）を2回連続で全通過 |
