...
| 13 | 決定表#6 は「記号の**形**は4スキン共通、変わるのは寸法と線の太さだけ」と決めた（#11 も同じ） | **形もスキンが選ぶ**。OUT コイルは4スキンとも**丸 `-○-`**（丸括弧をやめた）、SET / RST / MC / MCR / END は GX Works3風が**角括弧 `[SET Y0]`**・他3スキンが**命令ボックス**、タイマ／カウンタは GX Works3風が丸コイル＋設定値（`T0 K30`）・他3スキンが命令ボックス（命令語＋オペランド3行）。接点の縦棒の間隔は**セル幅の 15〜20%**（`SkinCell.contactGapPx`）、モニタ表示は GX Works3風が帯（`block`）・他3スキンがパワーフロー（`flow`）。あわせて左に行番号欄、分岐の接合点、END の専用記号を足した | **利用者（電気系保全の指導員）の 2026-09-20 の指摘**「PLCやヒントの回路図のシンボルが変。PLCは各メーカのソフトの画面表示と仕様に忠実に」「出力が四角や `()` で表されていて丸でないのはおかしい。すべてのシンボルのA接点やB接点の縦棒の間隔がやや広いのも気になる」。決定表#6 が形を共通にしたのは「一次資料の無い△を増やさない」ためだったが、**実物と違って見えること自体が教材の不具合**なので利用者判断が勝つ。形を選ぶ値（`instructionStyle` / `timerStyle` / `monitorStyle` / `contactGapPx` / `coilRxPx` / `stepGutterPx`）はすべて `ladder/skins/*.ts` に置いたので、「実機と違うと分かったら1ファイル」（§17.1）は保たれている |
| 14 | 決定表#6 / 上の #13 は接点の縦棒の間隔を「**セル幅の 15〜20%**」、記号の高さを「セル高の 45〜55%」、線幅を 1.4〜1.8px、行の高さを 46〜60px と決めていた | **利用者が示した三菱の命令記号表の比で描き直す**（2026-09-20 2回目）。縦棒の高さ＝**セル高の 35〜40%**、縦棒の間隔＝**縦棒の高さの約半分**（`contactGapPx`。記号表の実測は 10 : 16）、線幅は**全スキン 1.2px**、行の高さは GX Works3風 34px / PCwin風・JW-300SP風 36px / CX-Programmer風 48px、左母線 3px・右母線 1.2px・行番号欄 24px、b接点の斜線は**縦棒2本の x を左右に貫く1本**（`NC_OVERSHOOT_RATIO` / `NC_HEIGHT_RATIO`）、微分接点の矢印は**線画で軸の高さ＝縦棒の高さ**（`pulseGapPx` だけ間隔を広げる）、行番号は**回路ブロックの先頭行にだけ**（CX-Programmer風はラング番号。`SkinTheme.stepNumbering`）、コメントの字は 9px（CX-Programmer風の2行だけ 8px。`SkinCell.commentFontPx`）。**END の行はふつうの桟**（左母線から出力列まで1本）で、出力列に `[END]`／END の箱を置き、`END_MARK`（裸の縦棒2本）は削除した | **利用者（電気系保全の指導員）の 2026-09-20 の再指摘**「A接点やB接点の縦線の幅がやや広いのとシンボルのデザインがおかしいのが直ってない」「2本の縦線の間隔がまだ広い」「ラダー全体を各社ソフトの実画面にもっと寄せる」「PLCの回路画面のENDの行のデザインが実物と違う／END行の縦線2本は何？」「B接点シンボルの斜め線の位置おかしいんだけど」。利用者が提示した三菱の命令記号表（LD / LDI / AND / ANI / OR / ORI / LDP / LDF）から**比だけ**を読み取って作図した（表そのものは複製していない。§17）。CX-Programmer風だけ 48px なのは、I/Oコメント2行を縦棒の下に置くと 44px では次の行のデバイス名にかかるため（2行×8px＋縦棒18px＋デバイス名11px が収まる最小の高さ） |
