...
| 記録 | 進捗・成績の永続記録はしない。結果表示と、利用者が選んだ場所への1回の書き出しのみ |
| UI言語 | 日本語のみ |
...
| 5 | 進捗記録はしない。手動の作業ファイル保存/読込と、クラッシュ復帰用の一時保存のみ | 成績管理はOJT運用側の責務。アプリは貯めず、利用者が選んだ場所への結果1回の書き出しだけを認める | §12.3、§13 の #7・#8 |
...
**Phase 7 Task 41: IPC 9本目** `result:export` は `{ html, suggestedName }` を受け取り、OSの保存ダイアログで選んだ場所へ今回の結果だけをPDFまたはHTMLとして書き出す。HTMLは1 MiBまで。PDFはJavaScript・ネットワーク・権限を無効にした印刷専用窓で生成し、一時ファイルと窓を必ず閉じる。キャンセルは成功・取消として返し、ファイルは作らない。履歴・既定保存先・利用者名はアプリに保存しない。
...
| 数値解法 | 解法は毎tick 密行列を組み直す素のガウス消去である（節点162で 408 KiB/tick）。LU の再利用は行っていない。性能の実測が必要になった時点で `Solver` クラス化（`fill(0)` での使い回し）を検討する |
...
| 電線の選択・削除 | 電線はクリックで選択し、Delete で削除する（2026-09-24 改訂: 以前は削除モードのときのみ。利用者報告「選択してDeleteを押しても外れない」）。配線モードでは端子クリックを優先し、同じ点に端子・部品があればそちらを拾う。配線中（始点を選んだ後）に電線を押しても選択しない。配線モードでは固定配線を選ばない。電線一覧で選んだ電線もモードを問わず Delete で削除できる。Esc・何もない所のクリックで選択を解除する。削除モードで電線をクリックしても選択できる。既設配線はホバーで『既設配線（削除不可）』を表示し、選択・削除できない |
...
| 進捗記録 | しない（決定事項#5）。アプリは結果を蓄積せず、利用者が明示的に選んだ場所へ今回の結果を1回だけ書き出せる |
...
| 3 | エンジンの数値的破綻（特異行列・NaN） | 各節点に 1nS の漏れ抵抗を入れているため特異行列は発生しない。したがって `solver-warning` ／ 直前解での置換 ／ 10tick 停止は実装しない |
...
| 文言 | 日本語のみ。ただし全文言を `apps/desktop/src/renderer/i18n/ja.ts` に集約し、ハードコードしない |
...
| 18 | アプリ名称 | 「電気教育ツール」（2026-09-19 の利用者決定。仮称「OJT電気保全トレーナー」から改称） | `apps/desktop/src/renderer/i18n/ja.ts`・`electron-builder.yml` の `productName`・`main/index.ts` の `%APPDATA%` フォルダ名・`build/license.txt`・`docs/manual/*.md` など、ソース／ビルド設定／取扱説明書に分散して**13ファイル**が製品名の文字列を直接持つ（`PRODUCT_NAME` 定数のような単一の経由点は未導入。導入すればこの13ファイルを1箇所に集約できる） |
...
| 2026-09-20 | Phase 3〜6 の実装（`ladder-core` とラダーIR、4方言の `plc-dialects`、回路図エディタ、electron-builder 配布、取扱説明書PDFとアプリ内ヘルプ）を反映し、一次資料の記述を実態に合わせた（QA-20。Phase 7 Task 33）。§14.2 の E2E 行を実際の spec 構成（12 spec／67テスト）に、§15 の文言集約先を `apps/desktop/src/renderer/i18n/ja.ts` に、§17.2 #18 の製品名の出現箇所を13ファイルに直した。§5.2・§13 #3 は Task 14（2026-09-20）が別途訂正済み |
