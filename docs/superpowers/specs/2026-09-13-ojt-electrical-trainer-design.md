...
| 拡張子 | `.ojtw` |
| 一時保存 | 30秒ごとに Electron の `userData` ディレクトリ直下の単一ファイル `userData/autosave.json`（Windows では `%APPDATA%/OJT電気保全トレーナー/autosave.json`）へ上書き保存する。ファイルは常に1つのみで世代は持たず、復元対象は常に1件である |
