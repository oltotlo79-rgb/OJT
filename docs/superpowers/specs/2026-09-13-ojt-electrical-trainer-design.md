...
| UI言語 | 日本語のみ |
| 作業名（仮称） | **「OJT電気保全トレーナー」**。本名称は仮称であり、正式名称の決定により変更されうる。変更時の影響範囲は §17 の表に記す |
...
| BZ | ブザー。JIPM標準構成には含まれない任意部品 | §5.3 参照 |
...
| チェック用ソケット | 盤上5個目の14ピンソケット。赤PBのa接点でコイルを励磁する黄色の固定配線が施されている。部品単体の良否点検に使う | 調査資料 §3.5 |
| 限時動作瞬時復帰接点 | オンディレータイマの接点。通電後に設定時間で反転し、通電断で即時復帰する | 調査資料 §3.4 |
| 操作列 | 課題ごとに定義された、時刻付きのPB操作の列。判定時に模範回路と訓練者回路へ同一に与える | §7.4 |
| 模範回路 | 課題が持つ正解の回路（展開接続図またはラダーIR＋配線）。固定の正解波形は持たず、都度シミュレートする | §7.3 |
...
| 静的チェック | 通電せずに回路の構造だけで判定できる検査（線色、1端子の本数、未使用部品、禁則回路、2段結線、PLC電源独立、I/O割付準拠） | §7.5 |
...
| 5 | 進捗記録はしない。手動の作業ファイル保存/読込と、クラッシュ復帰用の一時保存のみ | 成績管理はOJT運用側の責務。アプリはデータ保持責任を持たない | §12.3、§13.5 |
| 6 | Windows専用・完全オフライン | 工場内PCの運用実態に合わせる | §15 の配布方式、自動更新なし |
| 7 | 配線操作は「線色選択 → 端子クリック → 端子クリック」。経路はダクト経由で自動生成 | 3D空間でのケーブル引き回し操作は学習目的に寄与せず操作難度だけが上がる | §6.6、§8.2 |
| 8 | 合否判定は「模範回路と訓練者回路を同じ操作列で同じエンジンにかけ、出力波形を許容差付きで比較」。固定の正解波形は持たない | 検定の正答は非公開（調査資料 §5.7）であり、また課題追加時に波形を手作業で用意させない | §7.5、§8.3、§10.7 |
...
| 14 | PLCの基準機種は 三菱 MELSEC iQ-F FX5U + GX Works3。初回リリースの方言は 三菱／JTEKT TOYOPUC-Plus PC10P・PC10PE（PCwin）／OMRON CP1E・CP1L（CX-Programmer）／シャープ JW-300（JW-300SP） | 検定はメーカー不問（調査資料 §7.3）であり、受講者の自社環境に合わせた練習需要がある | §10.5、§16 の Phase 3/4 |
...
| 18 | 制御回路はDC24V。AC100Vはブレーカ1A→DC24V電源の一次側のみで配線済み | 調査資料 §2.3 で確定。回路図母線は `P(+24V)` / `N(0V)` | §5.2、§6.1 |
...
| 規則 | 内容 |
|---|---|
| `board-model` はデータ定義のみ | `circuit-sim` に依存しない。盤の定義は「部品カタログ＋配置＋端子」であり、電気的解釈はエンジン側が行う |
...
| Simulation Worker（Web Worker） | `circuit-sim` ＋ `ladder-core` ランタイムを10ms周期で実行 | `postMessage`。コマンド: `load` / `setActuator` / `injectFault` / `measure` / `setTimerPreset` / `run` / `pause` / `reset`。通知: `snapshot`（既定 50ms ごと＝5tickに1回にまとめて送出）、`event`（危険操作・保護動作・チャタリング検出） |
...
| PLC要素 | ネットリスト上の表現 |
|---|---|
| 入力 Xn | `PLC.SS` と `PLC.Xn` の間の抵抗要素（機種別。既定4.7kΩ、§5.1.3）。この要素に3mA以上流れると論理ONと判定する |
...
| Tの電源（コイル相当） | 650Ω | 36.9mA | CRコイルと同等として扱う（§17 前提） |
| PL（DC24V LED表示灯相当） | 2.4kΩ | 10mA | §17 前提 |
| BZ | 1kΩ | 24mA | §17 前提 |
...
| レアショート（`coil-layer-short`） | 抵抗値を正常値の30%（CRコイルなら195Ω）に下げ、かつ**励磁しきい値を定格の115%（27.6V）に引き上げる**。DC24V回路では引込電圧未達となり吸引しない。抵抗測定で195Ωが読めるため「動作しない＋抵抗が低い＝レアショート」「動作しない＋抵抗が無限大＝コイル断線」と切り分けられる（§17 前提）|
...
| 交流 | 扱わない |
| 規模上限 | 端子400・節点200を想定。1tickあたり1ms未満を目標とする |
...
チャタリングの検出条件: 判定区間内の任意の1秒窓で、同一信号の状態遷移が**10回以上**（5Hz以上）発生した場合をチャタリングとする。
...
| PB4 | 赤 | `PB4.c` / `PB4.a` / `PB4.b` | 同上。a接点は端子台 `TB_PB.4c` / `TB_PB.4a` を経由してチェック用ソケットの黄色固定配線に使われている（§6.3） |
...
| ID | 色 | 端子 | 備考 |
|---|---|---|---|
| PL1 | 白 | `PL1.+` / `PL1.-` | DC24V用。2.4kΩ。端子電圧19.2V以上で点灯表示 |
| PL2 | 黄 | `PL2.+` / `PL2.-` | 同上 |
| PL3 | 緑 | `PL3.+` / `PL3.-` | 同上 |
| PL4 | 赤 | `PL4.+` / `PL4.-` | 同上 |
...
| 逆手順 | `event: 'power-sequence'` を発行し危険操作として回数記録する。電源自体は通常どおり入切される（練習は中断しない） | 決定事項#12 |

### 5.4 故障注入API

```ts
injectFault(target: FaultTarget, kind: FaultKind, param?: number): void
...
| `coil-layer-short` | CRコイル | — | レアショート（§5.1.3） |
| `lamp-open` | PL／BZ | — | 断線 |
...
| Ω | プローブ間の等価抵抗 [Ω] | **電源OFF時のみ有効**。プローブ間に試験電圧1Vを印加し、流れる電流から等価抵抗を算出する。回り込み経路を含む値になるため「リレーを抜いて測る」教育がそのまま成立する（調査資料 §6.5(A)）。電源ON時は §5.6 の危険操作を発行し、読値は `----`（測定不能表示）とする |
| CONT（導通） | 抵抗値 ≤ 50Ω でブザー音（WebAudio合成）＋「導通」表示。> 50Ω で無音＋「−−−」 | Ωレンジと同じ条件・同じ制約 |
...
|---|---|---|---|
| 1 | プローブ間電圧が **1V以上**の状態でΩ／導通レンジを使用 | `live-ohm` | テスターのヒューズ溶断・誤測定（調査資料 §6.5 設計メモ） |
| 2 | アナログテスターで指示値がレンジ上限を超える | `analog-overrange` | 針の振り切れ・可動コイル損傷 |
| 3 | 短絡状態（電源保護が動作する状態）で電源ON | `short-power-on` | 電源保護動作。ブレーカ／電源の損傷 |
| 4 | 電源ON/OFFの手順違反（§5.3.5） | `power-sequence` | 突入電流・アーク |
| 5 | 1端子に3本目を接続しようとした（接続は拒否される） | `terminal-overload` | ネジ端子の保持不良・脱落（調査資料 §4.5、上限2本は本アプリの前提 §17） |
...
| 要素 | 数量 | 仕様 | 根拠 |
|---|---|---|---|
| 14ピンソケット | 5 | 4c用。うち4個が CR/T 用、1個がチェック用 | §2.1, §2.2 |
...
| P/N供給端子 | P.1〜P.6 / N.1〜N.6 | DC24V電源の出力端子群。内部で同電位 | §2.2（DC24V電源の −/+ 端子） |
| ブレーカ | 1 | 1A、AC一次側 | §2.1 |
| 電源スイッチ | 1 | AC一次側 | §2.4 |
| DC24V電源 | 1 | 配線済み。端子は測定可 | §2.1, §2.3 |
| ダクト | — | 配線経路。自動経路生成が参照する | 3D再現のための本アプリ要素 |
...
|---|---|
| 線色 | 黄 |
| 電線 | `P.6 → TB_PB.4c`、`TB_PB.4a → CHK.14`、`CHK.13 → N.6` の3本。PB4本体ではなくPB用端子台（12P）側に接続する（盤上の配線可能な端子は端子台側のため。§6.4） |
| 属性 | `locked: true`（変更不可）。削除・付け替えを試みると「チェック用回路の黄色配線は変更できません」と表示する |
| 根拠 | 調査資料 §3.5「受検者はこの黄色配線を変更してはならない」 |

`TB_PB.4c` と `TB_PB.4a` にはこの黄色配線が各1本ずつ既に接続されているため、同端子には追加で1本まで配線できる（1端子2本まで、§6.6）。`P.6` と `N.6` も同様に残り1本である。
...
| 電源スイッチ | `SW` | `SW.1`, `SW.2` |
| BZ（任意部品） | `BZ` | `BZ.+`, `BZ.-` |
| PLC本体 | `PLC` | `PLC.X0`, `PLC.Y0`, `PLC.COM0`, `PLC.SS`, `PLC.L`, `PLC.N` |
...
| 配置の規定 | 調査資料 §2.2 の配置図どおりに縦横比を保つ。上段: 受検番号シール位置／DC24V電源／ブレーカ／AC100Vプラグ、中段: 14ピンソケット5個（左から CR1, CR2, CR3(=T1), CR4(=T2)、右にチェック用）、その下: ランプ用端子台(8P)と PB用端子台(12P)、最下段: PL4個（白黄緑赤）と PB4個（黒黄緑赤） |
| 具体座標 | Phase 1 の実装で調査資料 §2.2 の配置図に基づき確定する（範囲決定）。本仕様では相対配置と縦横比の保持のみを規定する |
| ダクト | 盤面上のポリライン列として定義する。各セグメントは幅と高さを持ち、配線の自動経路が参照する |
| 端子の当たり判定 | 各端子に半径4mmの円形ピック領域を与える。ソケットの4段配置（§6.2）をそのまま再現する |
...
| 規則 | 内容 |
|---|---|
| 1端子の本数上限 | **2本まで**（§17 前提）。訓練者が3本目を接続しようとした時点でその接続を拒否し、`terminal-overload` イベントを発行して危険操作としてカウントする（§5.6）。課題JSONや作業ファイルから読み込んだ配線が上限を超えている場合は接続を保持したうえで静的チェック `terminalLimit` のエラーとする |
| 経路 | 自動。端子 → 最寄りダクト入口 → ダクト内 → 目的端子の最寄り出口 → 端子。ダクト内で重なる線は幅方向に並列オフセットして描く |
...
| `timeLimit` | `{standardMin, cutoffMin}` | 標準時間／打切り時間（分）。既定は課題1形式 `{50, 60}`、課題2形式 `{30, 50}`（調査資料 §1.2） |
| `board` | `{boardId, socketRoles, extraParts?}` | 盤ID、5ソケットの役割割当（`["CR1","CR2","T1","T2","CHK"]` 等）、任意追加部品（`["BZ"]` 等） |
...
```json
"judge": {
  "compare": ["PL1", "PL2", "PL3", "PL4"],
  "tolerance": { "edgeMs": 200, "ratio": 0.10 },
  "staticChecks": {
    "wireColor": true, "terminalLimit": true, "coilPolarity": true, "unusedParts": true,
    "forbiddenCircuit": true, "twoStage": false, "plcPowerIndependent": false, "ioAssignment": false
  }
}
```
...
| 項目 | 既定 | 説明 |
|---|---|---|
| `compare` | 盤に実在する出力部品すべて（標準盤では `PL1`〜`PL4`。課題が `BZ` を追加した場合は `BZ` を含む） | 比較対象の出力信号 |
...
| 静的チェック | 内容 | 既定で有効なモード |
|---|---|---|
| `wireColor` | 線色ルール（新規=青／修復=白／チェック回路=黄、調査資料 §4.2） | 全モード |
...
| `unusedParts` | 装着したが回路に組み込まれていない部品の検出 | 全モード |
| `forbiddenCircuit` | 禁則回路（タイマ自己遮断ワンショット／タイマ2個フリッカ）の検出。チャタリング検出（§5.3.2）と構造パターン照合の両方で判定 | B, C2 |
...
`truth` は `normal` ＋ 不良原因6種（`coil-open`／`coil-layer-short`／`a-open`／`a-weld`／`b-open`／`b-weld`）のいずれか（調査資料 §6.2）。タイマには `coil-layer-short` を出題しない（§17 前提）。

### 7.6 PLC課題の定義

| フィールド | 型 | 説明 |
|---|---|---|
| `plc` | `{vendor, model}` | `vendor` は `mitsubishi`／`jtekt`／`omron`／`sharp`。`model` は `FX5U`／`PC10P`／`CP1E` 等 |
...
検定での使用範囲: 1級は入力3点・出力4点なので X0〜X2 と Y0〜Y3、2級は入力3点・出力3点なので X0〜X2 と Y0〜Y2 を使う（調査資料 §1.1, §5.2）。X3 は4入力を使う独自課題のための予備であり、内蔵課題では使用しない。X3 を使う場合、`TB_PB.4c` / `TB_PB.4a` にはチェック用の黄色配線が既に1本あるため、追加できるのは1本のみである（§6.3）。
...
| 利用者追加 | 設定で指定。既定 `%APPDATA%/OJT電気保全トレーナー/content/` |
...
| モード | 内容 | 題数 |
|---|---|---|
| B（回路組立） | 基本回路（a接点／b接点／AND／OR／自己保持／インターロック／オンディレー／ワンショット）（調査資料 §5.5） | 8 |
...
選択できる線色はモードで決まる。新規配線に使えるのは、モードB・モードDでは**青のみ**、モードC2では**白のみ**である。黄はチェック用回路専用のためいずれのモードでも選択できない（調査資料 §4.2）。

### 8.2 操作

| 操作 | 仕様 |
|---|---|
| 端子ホバー | `CR1 ⑨ COM` の形式でツールチップを出す（部品ID＋丸数字端子番号＋役割） |
| 配線 | 線色選択 → 端子クリック → 端子クリックで電線を生成。両端にY型圧着端子の見た目を付ける |
| 配線の取消 | 1本目クリック後に Esc または空間クリックで取り消す |
| 電線の選択・削除 | 電線クリックで選択、Delete で削除。`locked` の電線は選択できるが削除できない |
...
| 操作 | 部品をチェック用ソケットに挿す → 赤PB（PB4）を押して励磁 → テスターで測定 → 部品を外して次へ |
| 測定1 | コイル抵抗。**電源OFF・部品取り外し状態**で測るのが正しい。装着したまま測ると回り込み値になる（エンジンがそのまま再現する。調査資料 §6.5(A)） |
| 測定2 | 接点の導通。励磁ON時／OFF時の a接点・b接点 |
...
| コイル抵抗が約650Ω | 正常 |
| コイル抵抗が650Ωより著しく低い（本アプリでは195Ω） | レアショート |
| コイル抵抗が測定不能（OL） | コイル断線 |
...
| レンジ不足 | 指示値がレンジ上限を超えたら針を振り切れ位置に固定し、`analog-overrange` 警告を出す（§5.6） |
...
| OMRON CP1E-N30DR-A | 一体形・千鳥2列ネジ端子台 | 130 × 90 × 85 mm（取付穴ピッチ120mm） | `L1` / `L2/N`（**入力端子台側に同居**） | `COM` ＋ `0.00`〜`0.11`（12点）、`1.00`〜`1.05`（6点）の計18点 | `100.00`〜`100.07`、`101.00`〜`101.03` の計12点 ＋ `COM`×5。出力端子台に DC24Vサービス電源 `+` / `−` が同居 | 【本アプリの前提】POWER / RUN / ERR / ALM ＋ 入出力表示LED（PLC調査資料 O-3 未確認） |
| JTEKT TOYOPUC | **ラック形**（PC10P はPCIバス基板のため単体では盤横に置けない）。基本ベース＋`POWER1`（AC85〜264V）＋`IN-12`（DC入力16点）＋`OUT-12`（リレー出力16点）のラック構成として描く | モジュールは着脱式端子台 | `POWER1` の AC入力端子 | `IN-12`: DC24V 16点、**8点/COM** | `OUT-12`: リレー16点、AC240/DC24V、2A/点 | 入力ON時LED点灯（`IN-12`）。LEDモニタ |
...
|---|---|
| プログラム | ネットワークの列 |
| ネットワーク | セルグリッド（行×列）。**IRは最大の列数を持ち**、スキンが表示上の列数を狭める。IRの列数は16固定とし、スキンは11列（GX Works3風）等で表示する |
...
| 項目 | 三菱 FX5U | OMRON CP1E / CP1L | JTEKT TOYOPUC PC10P / PC10PE | シャープ JW-300 |
|---|---|---|---|---|
...
| 立上り／立下り微分 | `PLS` / `PLF`（出力形）、`LDP` / `LDF`（入力形）。GX Works3 では `Alt + /` で切換 | `DIFU` / `DIFD`（出力形）、`UP` / `DOWN`（接点形） | エッジ検出デバイス `P`。命令名はPLC調査資料 J-8 が未確認のため `PLS` / `PLF` を暫定表記とする（§17 #19） | `OUT POS`（`P`）/ `OUT NEG`（`N`）、接点は `STR POS` / `AND POS` / `OR POS` |
| セット／リセット | `SET` / `RST` | `SET` / `RSET` | 同上（暫定） | `SET`（`S`）/ `RST`（`R`） |
| 出力／ブロック接続 | `OUT`、`ANB` / `ORB` | `OUT` / `OUT NOT`、`AND LD` / `OR LD` | 同上（暫定） | `OUT` / `OUT NOT`、`AND STR` / `OR STR` |
| 区間制御（MC/MCR） | `MC` / `MCR` | `IL` / `ILC` | 同上（暫定） | `F-47`(ONLS) / `F-48`(ONLR) |
| END | `END` | `END` | 同上（暫定） | `F-40` |
| 常時ON | `M8000`（調査資料 §8.1） | `P_On` | `1V` 帯（PLC調査資料 J-7 未確認のため `1V00` を暫定割当） | `007366` を b接点で使用 |
| 初期パルス | `M8002`（調査資料 §8.1） | `A200.11` | 同上（`1V01` を暫定割当） | `007362` |
| 1秒クロック | `M8013`（本アプリの暫定割当。PLC調査資料 M-5 が未確認） | `P_1s` | `V072`（PLC調査資料 §5-3 はPCwinの画面写真からの読み取りのため、本アプリの暫定割当として扱う） | `007364` |
...
「暫定」と記した命令名・特殊デバイス番号は、一次資料が入手できていないための**本アプリ固有の割当**であり、UIには常に方言名を表示したうえで、設定画面に「一部の命令名は実機マニュアル未確認のため本アプリの表記です」と注記する（§17 #19）。

### 10.6 スキン

| スキン | キー割当（◎ = 一次資料で確認済み、○ = 二次資料、△ = 本アプリ独自の割当） | 画面構成・操作フロー |
|---|---|---|
| GX Works3風（三菱） | ◎ `F5` a接点／`F7` コイル／`F8` 応用命令／`/` a・b接点切換／`Alt + /` 微分・SET/RST切換／`F2` 書込みモード（読出し時）・簡易編集（書込み時）／`Shift+F2` 読出しモード／`F3` モニタ／`Shift+F3` モニタ（書込み）／`Ins` 挿入・上書き切換／`Ctrl + ←↑↓→` 罫線／`Tab` 次の回路記号／`F1` ヘルプ（PLC調査資料 §1-D）。△ `F6` b接点／`Shift+F5` OR a接点／`F9` 横線／`Shift+F9` 縦線／`F4` 変換（決定事項#15。PLC調査資料 M-7 が未確認のため本アプリの割当） | ナビゲーションウィンドウ（プロジェクトツリー）＋ラダーエディタ＋出力ウィンドウ。**変換が必要**（［変換］→［全変換］→結果は出力ウィンドウ）→［オンライン］→［シーケンサへの書込み］→ `F3` モニタ |
| CX-Programmer風（OMRON） | ○ `C` a接点／`/` b接点／`O` コイル／`I` 命令入力／`Ctrl+E` オンライン編集／`Ctrl+Shift+E` 転送（PLC調査資料 §5-4）。△ `W` 横線／`L` 縦線（決定事項#15。PLC調査資料 O-6 が未確認） | プロジェクトツリー＋ラダー編集＋出力ウィンドウ。**オフライン編集では変換が不要**（`convertStep: false`） |
| PCwin風（JTEKT） | △ GX Works3風と同一のキー割当を既定とする（PLC調査資料 J-10 が未確認のため。スキン定義ファイルの1箇所で差し替えられる） | 左にプロジェクトツリー（プログラム／データファイル／パラメータ／LD／SFC）、右にラダー編集エリア、上部にメニューバーとツールバー（`JP1` `DGR` `MOB` `STP` `RDY` `RUN` `RES` 等）、下部にステータスバー。**スクリーンエディタ方式**（`convertStep: false`）。モニタは LD追っかけモニタとタイムチャートモニタ（サンプリング0.1s・最大600s・最大32点）を模す |
| JW-300SP風（シャープ） | △ GX Works3風と同一のキー割当を既定とする（PLC調査資料 S-6 が未確認） | プロジェクトツリー＋ラダー編集＋出力ウィンドウ。変換の要否が不明のため `convertStep: true` を既定とする |
...
ラダーのグリッド列数とモニタ中の通電表示色は4機種とも一次資料が未確認である（PLC調査資料 G-1, G-2）。本アプリは**表示11列・通電色を明るい青**を全スキン共通の既定とし、`DialectProfile.gridCols` と `monitorColors` で機種ごとに差し替えられる形にする（§17 #19）。
...
|---|---|
| 構造 | グリッド。**列＝縦の枝**、**行＝段** |
| 向き | 既定は縦書き（上が P、下が N）。横書き切替は**表示のみ**で、文書モデルは常に縦書きで保持する |
...
| 母線表記 | 左（上）が `P(+24V)`、右（下）が `N(0V)`（調査資料 §8.4） |
...
| P/N | 母線 → `P.1`〜`P.6` / `N.1`〜`N.6` のうち、1端子2本の制約を満たすように若番から割り当てる |
| 上書き | `physicalOverride` があればその割当を優先する |
| 線色 | 生成された電線には課題種別に応じた色を付ける。モードB・モードD = 青、C2の初期配線 = 青、C2の修復 = 白、チェック用回路 = 黄 |
...
ホーム（モード選択: 回路組立／部品点検／回路点検・修復／PLC）
  └→ 課題一覧（級・タイトル・所要目安・読込エラー表示）
       └→ セッション画面（§8 / §9 / §10）
            └→ 結果画面
設定画面（既定メーカー、既定テスター種別、音のON/OFFと音量、利用者課題フォルダ）
```
...
| 項目 | 仕様 |
|---|---|
| 操作 | 軌道回転（左ドラッグ）、ズーム（ホイール）、平行移動（右ドラッグまたは中ドラッグ） |
| プリセット | 正面／俯瞰／ソケット拡大の3種。ツールバーとキー 1/2/3 で切替 |
| ピック | Three.js のレイキャストで端子・電線・部品を取得する |
...
| 項目 | 仕様 |
|---|---|
| 作業ファイル | JSON。任意の場所へ保存／読込する。内容は課題ID、盤の状態（装着部品・電線・タイマ設定）、ラダーIR、テスター状態、経過時間、危険操作カウンタ |
| 拡張子 | `.ojtw` |
| 一時保存 | 30秒ごとに `%APPDATA%/OJT電気保全トレーナー/autosave/` へ書き出す |
...
### 14.1 ゴールデンケース（Vitest、`packages/circuit-sim` ＋ `packages/ladder-core`）
...
| E2E | Playwright（Electron） | ①起動→課題選択→配線→通電→判定→結果 ②C2で故障を指摘して修復し合格 ③PLCでラダーを組んで配線して合格 ④作業ファイルの保存→再起動→読込 |
| カバレッジ目標 | `circuit-sim` / `ladder-core` / `schematic-core` / `content` は行・分岐とも **90%以上**。`apps/desktop` は目標を設けず、上記E2E4本の通過をもって可とする |
...
| オフライン | 実行時・起動時に外部通信を行わない。フォントも同梱する |
| 自動更新 | 無し。更新は新しいインストーラの配布で行う |
| 性能目標 | 内蔵GPU・FHD（1920×1080）で **60fps**。シミュレーションは1tick 1ms未満（§5.2） |
...
| 商標表示 | 設定画面内の「このアプリについて」に商標注記を常設する。MELSEC / MELSEC iQ-F / MELSOFT / GX Works3 は三菱電機、SYSMAC / CP1E / CP1L / CX-Programmer / CX-One はオムロン、TOYOPUC / PCwin / TOYOPUC-Plus はジェイテクト、JW / JW300 / JW-300SP はシャープの商標または登録商標である旨と、各社製品名は識別目的でのみ使用し提携・後援関係を示さない旨を明記する。各社のロゴ・アイコン・画面キャプチャ・図記号ビットマップは同梱しない（PLC調査資料 §6） |
...
|---|---|---|---|---|---|
| **1** | モノレポ雛形、`circuit-sim`（§5 の全機能のうち故障注入APIと測定APIの実装を含む。UIは無し）、`board-model`（JIPM標準盤）、`content` スキーマ＋モードB課題8題、回路図の**読取専用**レンダラ、`apps/desktop` 骨格、3D盤の配線・部品装着・タイマ設定・通電・PB操作、判定と結果画面 | テスター、故障注入UI、PLC、回路図編集、作業ファイル保存/読込、C1/C2 | 動作するデスクトップアプリ（モードBのみ） | ①アプリを起動しモードB課題「自己保持回路」を選び、3D盤上でCR1を装着し配線し、ブレーカ→スイッチで通電、黒PBで自己保持しPL1が点灯する ②「判定」で合格が表示され、タイムチャートが重ね表示される ③わざと1本外して判定すると不合格になり差分一覧に該当信号が出る ④`pnpm -r test` が全て通り、`circuit-sim` / `schematic-core` / `content` の行・分岐カバレッジが90%以上 | 2〜3日 |
| **2** | テスター（デジタル／アナログ）、故障注入の課題適用、危険操作の検知と記録、モードC1（課題4セット）、モードC2（課題8題）、C2での回路図連動ハイライト、作業ファイルの保存/読込、クラッシュ復帰、設定画面 | PLC、回路図編集、方言 | モードB・C1・C2 が動くアプリ | ①C1課題でチェック用ソケットに不良リレーを挿し、赤PBで励磁しテスターで測ると判定表どおりの読値になる ②マークシートパネルで「コイル断線」を選び判定すると正解になる ③C2課題で故障2箇所を指摘し白線で修復すると合格する ④電源ONのままΩレンジを当てると警告が出て結果に回数が記録される ⑤作業を保存し、アプリを再起動して読み込むと配線と装着が復元される | 2〜3日 |
...
| **4** | OMRON・JTEKT・シャープの方言プロファイルとスキン（ショートカット表・記号・画面構成・`convertStep`）、CP1E（一体形）とTOYOPUC・JW300（ラック形: ベース＋電源＋I/Oモジュール）の3Dモデル、表記切替、命令語リストのエクスポート、方言バリデータ | 回路図編集、配布パッケージ | 4方言が切替できるアプリ | ①設定で既定メーカーをOMRONにするとCX-Programmer風スキンで開き「変換」ボタンが出ない ②三菱で組んだラダーをOMRON表記に切り替えるとデバイス名が `0.00` 形式に変わる ③TOYOPUCでXとYに同番号を割り当てるとバリデータがエラーを出す ④シャープでリレー番号に `8` を入力すると8進エラーになる ⑤JW300のラック（電源＋CU＋`JW-212NA`＋`JW-214SA`）が3Dに出て `COM.A` へ配線できる ⑥命令語リストをテキストへ書き出すと方言どおりの命令名で出力される | 2〜3日 |
...
| 2 | 1端子への挿入本数の上限（未確認#3） | **2本まで**。3本目は静的エラー `terminal-overload` | §6.6 の規則、静的チェック、§11.3 の母線割当ロジック |
...
| 7 | レアショートの見え方（調査資料 §6.3 は「正常動作に見える場合がある」） | 本アプリでは**引込電圧未達として吸引しない**モデルにする（抵抗195Ω＋励磁しきい値27.6V）。動作しない部品を抵抗測定で「断線（OL）」と「レアショート（195Ω）」に切り分ける導線とする | §5.1.3 の負荷故障モデル、§9.1 のヘルプ表、ゴールデンケース#18 |
...
| 10 | シャープJW / JTEKT TOYOPUC の命令ニモニック（調査資料 未確認#12, #13／PLC調査資料 J-8, J-9） | シャープJWはPLC調査資料 §4-C から確定した（`STR/AND/OR POS·NEG`、`OUT POS/NEG`、`SET`/`RST`、`AND STR`/`OR STR`、`F-40`/`F-47`/`F-48`）。**JTEKT TOYOPUC の命令名のみ未入手**のため、三菱系の `LD`/`OUT`/`SET`/`RST`/`PLS`/`PLF` を暫定表記として実装する | `packages/plc-dialects` の `jtekt` プロファイルの `instructionNames` のみ。IR・ランタイム・他の3方言は変更不要 |
...
| 19 | 純正ツールのショートカット・グリッド列数・通電表示色（PLC調査資料 M-7, O-6, J-10, S-6, G-1, G-2） | §10.6 の表で「◎確認済み／○二次資料／△本アプリ独自」を明示し、△の割当をそのまま実装する。PCwin風・JW-300SP風はGX Works3風のキー割当を既定として流用する。グリッドは表示11列、通電色は明るい青を全スキン共通の既定とする。設定画面に「一部の命令名・キー割当は実機マニュアル未確認のため本アプリの表記です」と注記する | `packages/plc-dialects` の `shortcuts` / `gridCols` / `monitorColors` のみ。1プロファイル1ファイルで差し替えられる |
| 20 | 三菱FX5U・JTEKT のタイマ時間単位（PLC調査資料 M-4, J-9） | 三菱は調査資料 §8.2 のFX3系体系（既定100ms／`T200`〜10ms／`T256`〜1ms）、JTEKTは0.1s単位として実装する | `packages/plc-dialects` の `timerPreset` / `parseTimerPreset` のみ。IRはms保持のため課題データは無変更 |
| 21 | JTEKT TOYOPUC PC10P が一体形PLCではなくPCIバス基板である（PLC調査資料 §3-A） | 3D上は BUS-EXP 経由のラック構成（基本ベース＋`POWER1`＋`IN-12`＋`OUT-12`）として描く。ラダー方言は PC10P / PC10PE のまま変更しない | §10.1 の機種表と `packages/board-model` のJTEKT部品定義 |
| 22 | 三菱FX5U の特殊リレー番号（PLC調査資料 M-5） | 常時ON `M8000`・初期パルス `M8002` は調査資料 §8.1 で確定。1秒クロックは資料に無いため `M8013` を暫定割当とする。OMRONはPLC調査資料 §5-3 の `P_On`／`A200.11`／`P_1s`、シャープは `007366`／`007362`／`007364` で確定。JTEKTは `1V00`／`1V01`／`V072` を暫定割当とする | `packages/plc-dialects` の特殊デバイス写像テーブルのみ |
...
| 14 | PL・BZ・PLC入力の抵抗値（調査資料に記載なし） | PL = 2.4kΩ、BZ = 1kΩ、PLC入力 = 4.7kΩ（ON判定 3mA以上）。Tの電源はCRコイルと同じ650Ω | §5.1.3 |
...
| 18 | アプリ名称 | 仮称「OJT電気保全トレーナー」 | `resources/i18n/ja.json`、`%APPDATA%` 配下のフォルダ名、electron-builder の `productName`。3箇所のみ |
