...
| **アプリによる**進捗・成績の永続記録 | 決定#5「進捗・成績の永続記録はしない」。**結果の1枚書き出し（PR-14）は含む** —— アプリが貯めるのではなく利用者が選んだ場所へ1回書き出すだけなので反転にあたらない（2026-09-20 の所有者決定。§3.4） |
...
| D17 | **所有者の決定（2026-09-20、本書の初版に対する回答）** | ①課題は **72題のまま**（増やさない） ②ビューキューブは**左右も上下と同じ比喩に反転**する（§8.3-3） ③`sandbox: true` を **(a) 有効化**する（preload を CJS 出力に。配布物での起動確認つき。Task 8） ④3D の名札を **12px に上げ、本体仕様 §8.2 も 12px に改訂**する（Task 24・26） ⑤見送っていた **PR-12・PR-13・PR-14 を Phase 7 に入れる**（Task 39・40・41） | いずれも本書の初版が「所有者の判断が要る」として送った項目。PR-14 は決定#5 の境目にあたるが、**アプリが貯めるのではなく利用者が1回書き出すだけ**なので反転ではないと整理した（自動保存・履歴の蓄積は一切しない） |
...
| PR-12 判定の操作列を1歩ずつ再生する | 対応する | **Task 39**（2026-09-20 の所有者決定で Phase 8 から繰り上げ）。判定と食い違わないよう、再生は**判定と同じ操作列・同じ許容差**を使う |
| PR-13 模範と自分を並べて見る比較ビュー | 対応する | **Task 40**（同上） |
| PR-14 結果の1枚書き出し | 対応する | **Task 41**（同上）。決定#5 は「アプリが貯めない」ことなので、**利用者が選んだ場所へ1回書き出すだけ**なら反転にあたらない。自動保存・履歴の蓄積は一切しない |
...
### 8.2 実測で確定した原因（2026-09-21 訂正）

初版の「俯瞰プリセットは極角0」という仮説は誤りだった。Task 19 の Electron 実測は、
俯瞰のワールド極角が0.8897rad、13°傾斜した盤への面直視が約0.23radだった。
以前は下ドラッグで極角を減らし、下限0へ張り付いた結果、水平ドラッグでも位置が変わらなくなった。
実測と数値の詳細はプラン Task 19 Step 1・4を参照。

### 8.3 実装と受入条件

1. `OrbitControls.minPolarAngle = 0.02` で球座標の極への張り付きを防ぐ。
2. 上下・左右とも `+dy` / `+dx` の規則とする。**俯瞰から正面へは上に約105pxドラッグ**。
3. 面直プリセットは盤の傾きを基準とする極角π/2を維持する。1.1°ずらす案はP.1/N.1端子のピックを壊すため採用しない。
4. 上限では盤の裏側へ進めない。全方向が必ず動くという初版の条件は、上限のある操作では成立しない。
5. ドラッグ状態機械・描画・配置を分割し、2本目のポインタは無視する。

### 8.4 回帰テスト

- 7プリセット×4方向で角度と位置を検査。面直・下面から下へは上限に留まることも検査する。
- 傾斜した盤の上方向を使っても、端子P.1/N.1のピックを維持する。
- 実アプリで俯瞰から上へ105px引くと正面に近づき、200px引いて下限へ達した後も水平に動ける。
- 2本目のポインタが来ても、終了時に慣性設定を復元する。
...
| 2026-09-20 | 所有者の決定5件を反映（決定表 D17）。①課題は72題のまま ②ビューキューブは左右も反転（§8.3-3） ③`sandbox: true` を有効化 ④名札12px と本体仕様 §8.2 の改訂 ⑤**PR-12・PR-13・PR-14 を Phase 7 に採用**（§1.3・§3.4）。プランのタスクは 38 → **41** になった |
