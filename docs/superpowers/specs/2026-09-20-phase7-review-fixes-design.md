...
3. **左右も同じ比喩に合わせて反転する**（2026-09-20 の所有者決定）。上下を「カメラが指に付いてくる」（下へ引けばカメラが下へ回る）に改めるので、左右も同じにする（右へ引けばカメラが右へ回る＝`azimuth: +dx × k`）。2軸の比喩を必ず一致させ、`e2e/navigation.spec.ts:228-261` の期待値も同時に反転させる。
