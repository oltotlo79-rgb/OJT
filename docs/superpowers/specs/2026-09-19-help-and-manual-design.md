...
| アプリ内ヘルプへの図（スクリーンショット）の表示 | 決定表#9 |
...
| 9 | **アプリ内ヘルプに図（スクリーンショット）を出すか** | **出さない**。図のある節には「この節の図は『説明書（PDF）を開く』で見られます。」の1行を出す | 図は17枚・合計およそ6MBある。renderer に取り込むと asar がその分太り、ヘルプは幅420pxの引き出しなので 1280px の画面写真を縮めても端子の文字が読めない。図が要るときは PDF を開くのが早い。一致検査は**見出しと本文から図の行を除いて**比べる（決定表#12） |
...
| 12 | **一致検査で何を比べるか** | ①`manual-content.ts` が「いま正本から作り直した結果」と**バイト一致** ②`manual-content.ts` と `manual.html` の両方から取り出した**節ID・見出し・本文の素の文**（図の行を除く）が**完全一致** | ①だけだと「片方だけ古い」は防げるが「変換の道が2本に分かれた」ことは見えない。②を足すと、生成の経路が2つあっても人が読む内容は同じだと言い切れる |
...
| `docs/manual/images/*.png` | 吹き出しを描き込んだスクリーンショット17枚 | 入れる |
...
| `helpModule` | `manual-content.ts` の全文（`MANUAL_CHAPTERS` / `MANUAL_SECTIONS` / `MANUAL_SOURCES` を輸出する TypeScript） |
...
5. アプリ内ヘルプ用の `html` では図を「この節の図は『説明書（PDF）を開く』で見られます。」の1行に置き換える（決定表#9）。同じ図が PDF 側では `<figure><img …><figcaption>…</figcaption></figure>` になる。
...
| `session-board.png` | モードB の練習画面 | ①上の帯 ②手順の案内 ③3Dの画面 ④右の欄 ⑤「判定」 |
| `session-terminal.png` | 端子にさわったときの札 | ①札 ②端子の番号 |
| `view-cube.png` | ビューキューブ | ①立方体の面 ②XYZの玉 ③「全体表示」 ④「傾きを戻す」 |
| `socket-card.png` | ソケットの部品カード | ①「のせる」 ②「外す」 ③「入れかえる」 |
| `judge-result.png` | 結果の画面（合格） | ①合否 ②かかった時間 ③食い違いの表 ④自動で見つかった間違い |
| `timechart.png` | タイムチャートの拡大 | ①信号の名前 ②時間の目盛 ③たての合わせ線 |
| `c1-tester.png` | C1 のテスターとチェック用ソケット | ①つまみ ②読み値 ③黒い棒 ④赤い棒 |
| `c1-marksheet.png` | C1 のマークシート | ①部品の行 ②こわれ方の列 ③答えた数 |
| `c2-repair.png` | C2 の指摘欄と連動ハイライト | ①「指摘」 ②指摘の一覧 ③光っている端子 |
| `plc-ladder.png` | モードD のラダー編集 | ①ラダーの格子 ②キー割当の欄 ③「変換」 ④出力の欄 |
| `plc-monitor.png` | モードD のモニタ | ①通電している線 ②「モニタ停止」 ③値の一覧 |
| `plc-notation.png` | 表記の切替 | ①切替先のメーカー ②変わる名前の表 ③「適用」 |
| `schematic-editor.png` | 回路図エディタと検算 | ①パレット ②回路図の格子 ③「検算」 ④指摘の欄 |
| `settings.png` | 設定 | ①利用者課題フォルダ ②音 ③PLCの既定のメーカー |
| `help-drawer.png` | ヘルプの引き出し | ①検索欄 ②目次 ③本文 ④「説明書（PDF）を開く」 |

すべて PNG、**1枚 300KB 以下**、寸法は 1280×800（`shots.json` に `crop` のある図はその寸法）、フォルダ合計 6MB 以下。**すべてこのアプリの実際の画面**であり、作り絵・模式図・他社ツールの画面は1枚も含まない（§15、§17.1、決定表#24b）。
...
| 単体 | `test/manual-images.test.ts` | 画像の実在・寸法（`shot-geometry.json` の `crop` と照合）・300KB 以下・フォルダ合計・`shots.json` に無いファイルが紛れていないこと。**撮影の最後のタスクで入れる** |
...
| ⑦ | `docs/manual/images` の画像が**すべて本アプリの実際の画面**であり（`shots.json` に定義の無いファイルが1枚も無い）、説明する操作要素の上に**丸数字の吹き出しと枠**が描かれ、本文がその番号で場所を指している。1枚 **300KB 以下**で、寸法が `shot-geometry.json` の指定どおりである |
...
| 2 | §15「フォント・音声ファイルを同梱しない」 | PDF を1つ同梱する（5〜10MB） | §16 Phase 6 が求める成果物そのもの。フォントは埋め込まず OS 標準（Yu Gothic UI / Meiryo）で組む |
