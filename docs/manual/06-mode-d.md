...
この左半分の欄全体が「ラダーの画面」で、格子そのものが「ラダー図」、格子のまわりを含めた欄が「ラダーエディタ」です。左には「プログラム」という一覧があり、課題の「回路ブロック」（ラダー図のひとまとまり）が並びます。ラダーの格子をクリックすると、その場所にデバイスを入れる欄が出ます。
...
キーの割り当てはメーカーごとにちがいます。いま選んでいるメーカーの割り当ては、画面の右の「キー割当」の欄にいつも出ています。この欄の説明には「キー割当はメーカー（方言プロファイル）ごとに切り替わります。」と出ます。三菱の書き方では `F5` で a接点、`F7` でコイル、`F4` で変換、`F3` で動きを見る、`F1` でこの説明書が開きます。

4メーカーの表は、実際に画面へ出ている割り当てをそのまま載せています。キーの割り当てはメーカーごとに切り替わります。三菱の書き方（GX Works3風）の割り当ては次のとおりです。
...
| Ctrl+矢印 | 罫線（縦線・横線の作図） | 確かめた割り当て |
| F4 | 変換 | このアプリで決めた割り当て |
| / | a接点・b接点の切換 | 確かめた割り当て |
| Alt+/ | 微分・SET/RSTの切換 | 確かめた割り当て |
| F2 | 書込みモード | 確かめた割り当て |
...
図では、①が「モニタ開始」、②が「RUN」の切り替え、③が「デバイス初期化」、④が「スキャン回数」です。
