...
画面の上の帯のすぐ下には「手順」という案内が出て、いまどの段階かを教えてくれます。案内は配線（3D盤）・ラダー作成・変換・モニタ開始・RUN・判定の5つに分かれています。配線はいつでも行えるので「済」の印は付かず、ラダー作成→変換→モニタ開始・RUN→判定の順に「いまここ」が進みます。「変換」の操作が無いメーカーでは、この段は出ません。手順の「配線（3D盤）」を押すと配線画面へ、「ラダー作成」を押すと編集画面へ直接切り替わります。編集中の内容は保持されます。案内のすぐ下には、いま何をすればよいかのヒントが1行で出ます（最初は「まずラダーを作ります」）。
