...
三菱の例では、運転を許可する内部リレー `M0` と `SM412` のa接点を直列に置き、出力 `Y0` につなぎます。別の段では `M0` と `SM412` のb接点を直列に置き、`Y1` につなぎます。M0がONの間だけ2つの出力が交互に点灯し、M0がOFFになると両方が消えます。ほかのメーカーでは、上の表のクロックと、そのメーカーの内部リレー・出力番号に読み替えます。
