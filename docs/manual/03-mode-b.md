...
母線（P と N の2本）は、端子から端子へ渡してつないでいきます。読み方は「画面の見方」の「端子に触れると出る札」にあります。
...
右の欄の「部品」から、ソケットをクリックするとカードが出るので、載せたい部品を選んで「装着」を押します。課題で決まっている役割（CR1、T1 など）はソケットの上に書いてあります。
