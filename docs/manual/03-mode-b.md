...
電線は、つなぎたい端子をクリックしてから別の端子をクリックすると、1本つながります。1つ目を選んだ状態は「始点」と表示されます。つなぐのをやめたいときは `Esc` を押します。

外すときは、電線をクリックすると選べます。選んだら `Delete` を押すと外れます。選んだ電線は色が変わり、盤の左上の表示に「選択:」と出ます。端子の上を押すと配線の始点になるので、電線は端子から離れた途中を押してください。右の欄の「電線一覧・接続先の変更」で行を押して選んだ電線も、`Delete` で外せます。選ぶのをやめるときは `Esc` を押すか、盤の何もない所をクリックします。上の帯の「削除モード」を押してから電線をクリックしても選べます。固定電線は外せません。
...
標準盤では1つの端子につなぐ電線は2本までです。3本目の接続は受け付けず、つなぎ過ぎを試みた危険操作として記録します。別の母線端子へ渡すなど、1端子あたり2本以内になるよう配線してください。自由練習用の拡張盤では、課題に指定された本数上限と線色を使います。
...
標準課題は青線・1端子2本のルールです。自由盤の既定は青／白／黄・1端子4本で、課題作成者が本数と色を指定できます。対象は回路組立モードの拡張盤課題です。B087〜B090では追加押ボタンと表示灯の配線を練習します。
