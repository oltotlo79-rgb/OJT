...
1. 設定の「課題の導入・作成」を開き、近い課題を選んで「課題を複製」を押します。
2. 課題ID・課題名・操作と達成条件を変更します。別の課題として追加する場合は、既存の課題と違うIDにしてください。
3. 回路組立と回路点検・修復は「模範回路図を編集」、PLCは「模範ラダーを編集」を押します。部品点検は「部品点検の出題部品」で正解や不良条件を変更します。
4. 操作列・判定区間・PLCのI/O割付・故障条件を、作成した回路と合うように設定します。
5. 「課題を検証（模範の自己判定）」を押します。指摘があれば修正し、合格したら「検証してJSONを保存」で書き出します。
6. 「保存先を利用者課題フォルダに設定する」を押すか、JSONを既存の利用者課題フォルダへ置きます。課題一覧を開き直し、課題文に従って実際に解けることも確認してください。

### 模範回路図の編集

左の記号を選び、図のセルをクリックして配置・置換します。「段を追加」や「分岐にする」で直列・並列回路を作れます。タイマのコイルを選ぶと設定時間を変更できます。「元に戻す」「やり直し」で操作を取り消せます。

図の①は記号パレット、②は「段を追加」です。

![課題作成の模範回路図エディタ。](images/authoring-schematic.png)

「課題全体を検証」は、回路の割当と課題としての模範動作を確認します。空の段などの構造上の指摘は画面下に表示します。点検・修復の故障箇所は電線・部品のIDを参照するため、模範回路を変更したら故障条件も検証し直してください。

### 模範ラダーの編集

編集するセルを選び、記号ボタンで接点・コイル・応用命令を入力します。既存のセルはダブルクリックでも編集できます。回路ブロックや並列行の追加・削除、「元に戻す」「やり直す」を使えます。ENDは残します。

図の①は「回路ブロックを追加」、②は編集するセルです。

![課題作成の模範ラダーエディタ。](images/authoring-ladder.png)

「命令の直接入力」には `LDI X0` のように命令とデバイスを書けます。「デバイス」の欄には `X0` のようにデバイスだけを入力します。入力欄が開いている間はまだ確定していません。「確定」または「取消」で閉じてから、変換・検証・編集の終了へ進みます。未確定の入力がある場合は通常のアプリ終了も止めます。

「ラダーを変換」で接続と命令を確認します。指摘を押すと該当セルへ移動します。続いて「課題全体を検証」で、機種・I/O割付・操作列との整合と模範動作を確認してください。

### 下書きと配布用JSON

作図中も変更は下書きへ随時保存します。「編集を終える」または `Esc` で下書きを保持して戻ります。「この画面の変更を取り消す…」は、確認したうえで編集画面を開いた時点の内容へ戻します。未完成の回路は後から同じ編集画面で続きを作れます。

下書きは作成途中の控えで、課題一覧には追加されません。配布用JSONは模範の検証に合格した内容だけを書き出せます。「詳細JSONを編集」は、画面の入力欄にない高度な条件やデータ形式を直す場合に使います。
...
内蔵課題は回路組立100題、部品点検64題、回路点検・修復100題、PLC100題の計364題です。設定の「課題の導入・作成」で複製元を選び「課題を複製」を押すと、新しいIDの雛形を取り出せます。インストーラ版・ポータブル版とも同じ操作です。
...
5. 「模範回路図を編集」または「模範ラダーを編集」でお手本を作図します。ランダム故障など画面に入力欄がない条件は「詳細JSONを編集」で調整します。表・作図画面・JSONは同じ定義を編集しています。
...
- 「使える線色」を設定します。自由練習の既定は青・白・黄です。1端子の本数はどの盤でも2本までで、課題ファイルに3本以上を書いても2本として扱います。
