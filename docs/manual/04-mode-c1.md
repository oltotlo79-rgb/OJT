...
棒を置くと、3D図のチェック用ソケットの端子に黒と赤の棒が立ち、先の上に「黒 CHK.13」のような端子名の札が出ます。棒の根元には色の輪が付くので、盤を回しても、どちらの棒をどこに当てているかが分かります。右の欄の「黒プローブ」「赤プローブ」にも同じ端子名が出ます。

図では、①が黒プローブの札、②が赤プローブの札です。

![3D図に立つテスターの棒と端子名の札。](images/c1-probe-pens.png)
...
図では、①が「判定表（切り分けの手順）」、②が「チェック状況」です。

![画面の中の判定表。](images/c1-diagnosis.png)
